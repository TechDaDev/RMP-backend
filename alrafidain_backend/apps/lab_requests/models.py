...
class LabOrderRequest(BaseModel):
    ...
    patient = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        related_name="lab_order_requests",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["lab_order", "lab"]),
            models.Index(fields=["patient", "created_at"]),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["lab_order", "lab"],
                condition=Q(status__in=["pending", "quoted"]),
                name="uniq_active_pending_quoted_laborder_lab_request",
            ),
            models.UniqueConstraint(
                fields=["lab_order"],
                condition=Q(status__in=["accepted", "completed"]),
                name="uniq_accepted_completed_lab_request_per_order",
            ),
        ]
    ...
    def clean(self):
        if self.patient_id and self.lab_order_id and self.patient_id != self.lab_order.patient_id:
            raise ValidationError({"patient": "Patient must match lab order patient."})
...
class LabOrderRequestItem(BaseModel):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["lab_order_item"]),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["request", "lab_order_item"],
                name="uniq_lab_request_lab_order_item",
            )
        ]
