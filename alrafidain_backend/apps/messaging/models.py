    ...
    def clean(self):
        ...
        if self.consultation.status not in allowed_send_statuses:
            raise ValidationError(
                {
                    "consultation": (
                        "Messages are only allowed for accepted or doctor_responded consultations."
                    )
                }
            )
        ...
        if self.sender_id not in [
            ...
            self.consultation.assigned_doctor_id,
        ]:
            raise ValidationError(
                {"sender": "Sender must be consultation patient or assigned doctor."}
            )

        if (
            self.sender.user_type == UserType.PATIENT
            and self.sender_role != MessageSenderRole.PATIENT
        ):
            raise ValidationError({"sender_role": "sender_role must match sender user type."})

        if (
            self.sender.user_type == UserType.DOCTOR
            and self.sender_role != MessageSenderRole.DOCTOR
        ):
            raise ValidationError({"sender_role": "sender_role must match sender user type."})

        if self.sender.user_type not in [UserType.PATIENT, UserType.DOCTOR]:
            raise ValidationError(
                {"sender": "Only patient and doctor can send messages in this phase."}
            )
...
class MessageAttachment(BaseModel):
    ...
    def clean(self):
        if (
            self.uploaded_by_id
            and self.message_id
            and self.uploaded_by_id != self.message.sender_id
        ):
