...
class MyMedicalRecordView(APIView):
	...
	def get(self, request):
		...
		return success_response("Medical record retrieved.", data=PatientMedicalRecordSerializer(record).data)
...
class DoctorPatientMedicalRecordView(APIView):
	...
	def get(self, request, patient_id):
		if request.user.user_type != UserType.DOCTOR:
			return error_response("Only doctors can access this endpoint.", status_code=status.HTTP_403_FORBIDDEN)
...
class MedicalRecordEntryCreateView(APIView):
	...
	@extend_schema(summary="Create medical record entry", request=MedicalRecordEntryCreateSerializer)
	def post(self, request, record_id):
		...
		if request.user.user_type == UserType.PATIENT and record.patient_id != request.user.id:
			return error_response("Not found.", status_code=status.HTTP_404_NOT_FOUND)
		if request.user.user_type == UserType.DOCTOR and not doctor_can_access_patient_record(request.user, record.patient):
			return error_response("Not found.", status_code=status.HTTP_404_NOT_FOUND)
		if request.user.user_type in [UserType.PHARMACIST, UserType.LABORATORIAN]:
			return error_response("This role cannot create generic medical record entries.", status_code=status.HTTP_403_FORBIDDEN)
...
class MedicalRecordEntryConfirmView(APIView):
	...
	@extend_schema(summary="Confirm or reject medical record entry", request=MedicalRecordEntryConfirmSerializer)
	def post(self, request, entry_id):
		if request.user.user_type != UserType.DOCTOR:
			return error_response("Only doctors can confirm entries.", status_code=status.HTTP_403_FORBIDDEN)
		...
		return success_response("Entry status updated.", data=MedicalRecordEntrySerializer(entry).data)
...
class MedicalRecordEntryDeactivateView(APIView):
	...
	@extend_schema(summary="Deactivate medical record entry", request=MedicalRecordEntryDeactivateSerializer)
	def post(self, request, entry_id):
		entry = get_object_or_404(MedicalRecordEntry, id=entry_id, is_active=True)
		...
		if request.user.user_type == UserType.PATIENT:
			...
			return error_response("This role cannot deactivate entries.", status_code=status.HTTP_403_FORBIDDEN)
...
class SetBloodGroupView(APIView):
	...
	def post(self, request, record_id):
		...
		if request.user.user_type == UserType.PHARMACIST:
			return error_response("Pharmacists cannot set blood group.", status_code=status.HTTP_403_FORBIDDEN)
		...
		return success_response("Blood group updated.", data=BloodGroupRecordSerializer(blood_group_record).data)
...
class LaboratorianVerifyBloodGroupView(APIView):
	...
	def post(self, request, patient_id):
		if request.user.user_type != UserType.LABORATORIAN:
			return error_response("Only laboratorians can use this endpoint.", status_code=status.HTTP_403_FORBIDDEN)
		...
		return success_response("Blood group verified.", data=BloodGroupRecordSerializer(blood_group_record).data)
