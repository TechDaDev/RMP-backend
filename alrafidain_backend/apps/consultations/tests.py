...
class TaxonomyTests(TestCase):
	def setUp(self):
		...
		self.c2 = SymptomCategory.objects.create(name="InactiveCat", display_order=2, is_active=False)
		Symptom.objects.create(category=self.c1, name="Chest Pain", is_red_flag=True, is_active=True)
...
class RecommendationServiceTests(TestCase):
	def setUp(self):
		...
		self.s2 = Symptom.objects.create(category=self.cat, name="S2", is_active=True, is_red_flag=True)
		self.s3 = Symptom.objects.create(category=self.cat, name="S3", is_active=False)

		SymptomSpecialtyRule.objects.create(symptom=self.s1, specialty=MedicalSpecialty.CARDIOLOGY, weight=3, is_active=True)
		SymptomSpecialtyRule.objects.create(symptom=self.s1, specialty=MedicalSpecialty.INTERNAL_MEDICINE, weight=1, is_active=True)
		SymptomSpecialtyRule.objects.create(symptom=self.s2, specialty=MedicalSpecialty.CARDIOLOGY, weight=2, is_active=True)
		SymptomSpecialtyRule.objects.create(symptom=self.s2, specialty=MedicalSpecialty.DERMATOLOGY, weight=8, is_active=False)
		SymptomSpecialtyRule.objects.create(symptom=self.s3, specialty=MedicalSpecialty.DERMATOLOGY, weight=10, is_active=True)
...
class ConsultationFlowTests(TestCase):
	def setUp(self):
		...
		self.symptom = Symptom.objects.create(category=self.cat, name="Chest pain", is_red_flag=True, is_active=True)
		self.symptom2 = Symptom.objects.create(category=self.cat, name="Palpitations", is_active=True)
		SymptomSpecialtyRule.objects.create(symptom=self.symptom, specialty=MedicalSpecialty.CARDIOLOGY, weight=5)
		SymptomSpecialtyRule.objects.create(symptom=self.symptom2, specialty=MedicalSpecialty.CARDIOLOGY, weight=1)
	...
	def test_doctor_cannot_accept_already_accepted(self):
		...
		resp = self.other_doctor_client.post(f"/api/consultations/{c.id}/accept/", {}, format="json")
		self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
	...
	def test_assigned_doctor_can_create_response(self):
		...
		resp = self.doctor_client.post(
			...
			{"response_text": "Please do ECG.", "recommendation_type": DoctorRecommendationType.NEEDS_LAB_TEST},
			format="json",
		)
		self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
		c.refresh_from_db()
	...
	def test_audit_logs_for_accept_response_close(self):
		...
		self.doctor_client.post(
			f"/api/consultations/{c.id}/responses/",
			{"response_text": "Take rest", "recommendation_type": DoctorRecommendationType.GENERAL_ADVICE},
			format="json",
		)
	...
	def test_doctor_specialty_other_only_handles_other(self):
		...
		c.save()
		resp2 = self.other_doctor_client.post(f"/api/consultations/{c.id}/accept/", {}, format="json")
