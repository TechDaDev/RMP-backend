...
class LabOrder(BaseModel):
	...
	def update_status_from_items(self):
		now = timezone.now()
		if self.is_expired() and self.status not in (LabOrderStatus.CANCELLED, LabOrderStatus.FULLY_COMPLETED):
	...
	def clean(self):
		...
		if self.consultation.status not in valid_statuses:
			raise ValidationError("Lab order can only be created for accepted or doctor_responded consultations.")
		if self.doctor_id != self.consultation.assigned_doctor_id:
			raise ValidationError("Only assigned doctor can create a lab order for this consultation.")
		...
		try:
			doctor_approved = self.doctor.doctor_profile.verification_status == VerificationStatus.APPROVED
		except Exception:
	...
	def save(self, *args, **kwargs):
		...
		if not self.expires_at:
			self.expires_at = (self.qr_token_created_at or timezone.now()) + timedelta(days=LAB_ORDER_EXPIRY_DAYS)
...
class LabCompletionRecord(BaseModel):
	...
	def clean(self):
		...
		try:
			approved = self.laboratorian.laboratorian_profile.verification_status == VerificationStatus.APPROVED
		except Exception:
			approved = False
		if not approved:
...
class LabResult(BaseModel):
	...
	flag = models.CharField(max_length=20, choices=LabResultFlag.choices, default=LabResultFlag.UNKNOWN)
	...
	def clean(self):
		...
		if self.value_type == LabResultValueType.BLOOD_GROUP and not self.blood_group_value:
			raise ValidationError({"blood_group_value": "This field is required for blood group results."})
