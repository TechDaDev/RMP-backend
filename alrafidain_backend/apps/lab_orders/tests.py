...
class LabOrderCreationTests(TestCase):
	...
	def test_unassigned_doctor_cannot_create(self):
		response = auth_client(create_doctor("other@example.com")).post(self._url(), self._payload(), format="json")
	...
	def test_pharmacist_cannot_create(self):
		response = auth_client(create_pharmacist()).post(self._url(), self._payload(), format="json")
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

	def test_laboratorian_cannot_create(self):
		response = auth_client(create_laboratorian()).post(self._url(), self._payload(), format="json")
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
	...
	def test_patient_notification_created_without_test_names(self):
		response = auth_client(self.doctor).post(self._url(), self._payload(), format="json")
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		notif = Notification.objects.filter(recipient=self.patient, notification_type=NotificationType.LAB_ORDER).latest("created_at")
...
class PatientSafetyTests(TestCase):
	...
	def test_patient_cannot_scan_qr_endpoint(self):
		response = auth_client(self.patient).post("/api/lab-orders/scan/", {"qr_token": self.lab_order.qr_token}, format="json")
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class DoctorVisibilityTests(TestCase):
	...
	def test_other_doctor_cannot_view_full_lab_order(self):
		response = auth_client(self.other_doctor).get(f"/api/lab-orders/doctor/{self.lab_order.id}/")
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

	def test_doctor_can_cancel_if_no_items_completed(self):
		response = auth_client(self.doctor).post(f"/api/lab-orders/doctor/{self.lab_order.id}/cancel/")
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_cancellation_sets_pending_items_cancelled(self):
		...
		self.assertEqual(self.lab_order.items.filter(status=LabOrderItemStatus.CANCELLED).count(), 2)
...
class LaboratorianScanTests(TestCase):
	...
	def test_fully_completed_scan_returns_locked_no_items(self):
		self.lab_order.items.update(status=LabOrderItemStatus.COMPLETED, completed_at=timezone.now())
...
class CompletionTests(TestCase):
	def setUp(self):
		...
		self.consultation = create_consultation(self.patient, self.doctor)
		self.lab_order = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=2)
	...
	def test_approved_laboratorian_can_complete_pending_item(self):
		...
		response = self._complete(
			self.lab,
			[{"lab_order_item_id": str(item.id), "status": LabCompletionAttemptStatus.COMPLETED, "note": "Done"}],
		)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_completed_item_hidden_from_future_scan(self):
		...
		scan = auth_client(self.lab).post("/api/lab-orders/scan/", {"qr_token": self.lab_order.qr_token}, format="json")
	...
	def test_unavailable_attempt_keeps_item_pending_and_visible(self):
		...
		response = self._complete(
			self.lab,
			[{"lab_order_item_id": str(item.id), "status": LabCompletionAttemptStatus.UNAVAILABLE, "note": "Not available"}],
		)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		item.refresh_from_db()
		self.assertEqual(item.status, LabOrderItemStatus.PENDING)
		self.assertTrue(LabCompletionRecord.objects.filter(lab_order_item=item, status=LabCompletionAttemptStatus.UNAVAILABLE).exists())

		scan2 = auth_client(self.lab2).post("/api/lab-orders/scan/", {"qr_token": self.lab_order.qr_token}, format="json")
	...
	def test_partial_completion_sets_status_partially_completed(self):
		item = self.lab_order.items.first()
		self._complete(self.lab, [{"lab_order_item_id": str(item.id), "status": LabCompletionAttemptStatus.COMPLETED}])
		self.lab_order.refresh_from_db()
	...
	def test_all_items_completed_sets_status_fully_completed(self):
		...
		response = self._complete(
			self.lab,
			[
				{"lab_order_item_id": str(items[0].id), "status": LabCompletionAttemptStatus.COMPLETED},
				{"lab_order_item_id": str(items[1].id), "status": LabCompletionAttemptStatus.COMPLETED},
			],
		)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.lab_order.refresh_from_db()
	...
	def test_cannot_complete_same_item_twice(self):
		item = self.lab_order.items.first()
		self._complete(self.lab, [{"lab_order_item_id": str(item.id), "status": LabCompletionAttemptStatus.COMPLETED}])
		response = self._complete(self.lab, [{"lab_order_item_id": str(item.id), "status": LabCompletionAttemptStatus.COMPLETED}])
		self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

	def test_cannot_complete_item_from_another_order(self):
		other_order = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=1)
		foreign_item = other_order.items.first()
		response = self._complete(
			self.lab,
			[{"lab_order_item_id": str(foreign_item.id), "status": LabCompletionAttemptStatus.COMPLETED}],
		)
		self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
	...
	def test_audit_logs_created_for_completed_unavailable_fully_completed(self):
		...
		self._complete(
			self.lab,
			[
				{"lab_order_item_id": str(items[0].id), "status": LabCompletionAttemptStatus.UNAVAILABLE},
				{"lab_order_item_id": str(items[1].id), "status": LabCompletionAttemptStatus.COMPLETED},
			],
		)
	...
	def test_doctor_and_patient_notifications_created(self):
		...
		self.assertTrue(Notification.objects.filter(recipient=self.doctor, title="Lab test completed").exists())
		self.assertTrue(Notification.objects.filter(recipient=self.doctor, title="Lab test unavailable").exists())

		self._complete(
			self.lab,
			[{"lab_order_item_id": str(items[0].id), "status": LabCompletionAttemptStatus.COMPLETED}],
		)
		patient_notif = Notification.objects.filter(recipient=self.patient, title="Lab order fully completed").latest("created_at")
...
class RoleAccessTests(TestCase):
	...
	def test_pharmacist_cannot_access_lab_order_endpoints(self):
		...
		self.assertEqual(client.post("/api/lab-orders/scan/", {"qr_token": self.lab_order.qr_token}, format="json").status_code, status.HTTP_403_FORBIDDEN)
...
class LabResultCreationTests(TestCase):
	def setUp(self):
		...
		self.consultation = create_consultation(self.patient, self.doctor)
		self.lab_order = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=1)
		self.item = self.lab_order.items.first()
		auth_client(self.lab).post(
			f"/api/lab-orders/{self.lab_order.id}/complete/",
			{"items": [{"lab_order_item_id": str(self.item.id), "status": LabCompletionAttemptStatus.COMPLETED}]},
			format="json",
		)
	...
	def test_cannot_create_result_for_pending_item(self):
		pending_order = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=1)
	...
	def test_unapproved_or_wrong_roles_cannot_create_result(self):
		self.assertEqual(auth_client(self.unapproved_lab).post(self._url(), {"value_type": "text", "text_value": "x"}, format="json").status_code, status.HTTP_403_FORBIDDEN)
		self.assertEqual(auth_client(self.patient).post(self._url(), {"value_type": "text", "text_value": "x"}, format="json").status_code, status.HTTP_403_FORBIDDEN)
		self.assertEqual(auth_client(self.doctor).post(self._url(), {"value_type": "text", "text_value": "x"}, format="json").status_code, status.HTTP_403_FORBIDDEN)
		self.assertEqual(auth_client(create_pharmacist()).post(self._url(), {"value_type": "text", "text_value": "x"}, format="json").status_code, status.HTTP_403_FORBIDDEN)

	def test_value_type_validations(self):
		self.assertEqual(auth_client(self.lab).post(self._url(), {"value_type": "numeric"}, format="json").status_code, status.HTTP_400_BAD_REQUEST)
		self.assertEqual(auth_client(self.lab).post(self._url(), {"value_type": "text"}, format="json").status_code, status.HTTP_400_BAD_REQUEST)
		self.assertEqual(auth_client(self.lab).post(self._url(), {"value_type": "blood_group"}, format="json").status_code, status.HTTP_400_BAD_REQUEST)

		file_item = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=1).items.first()
		auth_client(self.lab).post(
			f"/api/lab-orders/{file_item.lab_order.id}/complete/",
			{"items": [{"lab_order_item_id": str(file_item.id), "status": LabCompletionAttemptStatus.COMPLETED}]},
			format="json",
		)
		self.assertEqual(
			auth_client(self.lab).post(
		...
		file_upload = SimpleUploadedFile("result.pdf", b"pdfcontent", content_type="application/pdf")
	...
	def test_audit_and_doctor_notification_on_result_creation(self):
		...
		self.assertTrue(Notification.objects.filter(recipient=self.doctor, title="Lab result submitted").exists())
...
class LabResultVisibilityAndWorkflowTests(TestCase):
	def setUp(self):
		...
		create_resp = auth_client(self.lab).post(
			...
			{"value_type": LabResultValueType.NUMERIC, "numeric_value": "10.5", "unit": "g/dL", "laboratorian_notes": "internal"},
			format="json",
		)
		self.result_id = create_resp.data["data"]["id"]

	def test_patient_cannot_see_unreleased_result(self):
		self.assertEqual(auth_client(self.patient).get(f"/api/lab-orders/results/{self.result_id}/").status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(auth_client(self.patient).get("/api/lab-results/my/").status_code, status.HTTP_200_OK)
	...
	def test_ordering_doctor_can_view_full_other_doctor_cannot(self):
		self.assertEqual(auth_client(self.doctor).get(f"/api/lab-orders/doctor/results/{self.result_id}/").status_code, status.HTTP_200_OK)
		self.assertEqual(auth_client(self.other_doctor).get(f"/api/lab-orders/doctor/results/{self.result_id}/").status_code, status.HTTP_404_NOT_FOUND)
	...
	def test_review_and_release_workflow(self):
		...
		release = auth_client(self.doctor).post(f"/api/lab-orders/doctor/results/{self.result_id}/release/", {}, format="json")
		...
		self.assertTrue(Notification.objects.filter(recipient=self.patient, title="Lab result released").exists())
		...
		self.assertEqual(auth_client(self.other_patient).get(f"/api/lab-results/my/{self.result_id}/").status_code, status.HTTP_404_NOT_FOUND)
	...
	def test_correction_flow(self):
		...
		self.assertTrue(Notification.objects.filter(recipient=self.doctor, title="Lab result corrected").exists())

		self.assertEqual(
			auth_client(self.other_lab).post(
				f"/api/lab-orders/results/{self.result_id}/correct/",
	...
	def test_link_to_medical_record(self):
		...
		self.assertEqual(entry.verification_status, MedicalRecordVerificationStatus.LABORATORY_CONFIRMED)
		self.assertEqual(entry.source_role, MedicalRecordSourceRole.LABORATORIAN)
		self.assertTrue(AuditLog.objects.filter(action="lab_result_linked_to_medical_record").exists())
		self.assertTrue(Notification.objects.filter(recipient=self.patient, title="Medical record updated").exists())
	...
	def test_blood_group_result_updates_blood_group_record(self):
		bg_order = create_lab_order_with_items(self.patient, self.doctor, self.consultation, item_count=1)
		bg_item = bg_order.items.first()
		auth_client(self.lab).post(
			f"/api/lab-orders/{bg_order.id}/complete/",
			{"items": [{"lab_order_item_id": str(bg_item.id), "status": LabCompletionAttemptStatus.COMPLETED}]},
			format="json",
		)
		create_resp = auth_client(self.lab).post(
			f"/api/lab-orders/items/{bg_item.id}/results/",
			{"value_type": LabResultValueType.BLOOD_GROUP, "blood_group_value": BloodGroup.O_POSITIVE},
			format="json",
		)
		...
		self.assertEqual(bg_record.verification_status, MedicalRecordVerificationStatus.LABORATORY_CONFIRMED)

	def test_role_restrictions_for_result_endpoints(self):
		self.assertEqual(auth_client(self.pharmacist).get(f"/api/lab-orders/results/{self.result_id}/").status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(
			auth_client(self.patient).post(
				f"/api/lab-orders/doctor/results/{self.result_id}/link-medical-record/",
				{},
				format="json",
			).status_code,
