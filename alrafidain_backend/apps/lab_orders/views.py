...
from apps.common.choices import LabResultStatus, LabOrderItemStatus, LabOrderStatus, UserType
...
class LabOrderCreateView(CreateAPIView):
	...
	def create(self, request, *args, **kwargs):
		...
		if not is_approved_doctor(request.user):
			return error_response("Only approved doctors can create lab orders.", status_code=status.HTTP_403_FORBIDDEN)
...
class PatientLabOrderListView(ListAPIView):
	...
	def get_queryset(self):
		...
		return LabOrder.objects.filter(patient=self.request.user).select_related("doctor", "consultation")

	def list(self, request, *args, **kwargs):
		if request.user.user_type != UserType.PATIENT:
			return error_response("Only patients can list lab orders.", status_code=status.HTTP_403_FORBIDDEN)
		serializer = self.get_serializer(self.get_queryset(), many=True)
...
class PatientLabOrderDetailView(RetrieveAPIView):
	...
	def retrieve(self, request, *args, **kwargs):
		lab_order = get_object_or_404(LabOrder.objects.select_related("doctor", "consultation"), id=kwargs["lab_order_id"])
...
class DoctorCancelLabOrderView(APIView):
	...
	def post(self, request, lab_order_id):
		...
		if lab_order.status == LabOrderStatus.CANCELLED:
			return error_response("Lab order is already cancelled.", status_code=status.HTTP_400_BAD_REQUEST)
		...
		return success_response("Lab order cancelled.", data=LabOrderDoctorDetailSerializer(lab_order).data)
...
class LaboratorianLabOrderScanView(APIView):
	...
	def post(self, request):
		if not is_approved_laboratorian(request.user):
			return error_response("Only approved laboratorians can scan QR tokens.", status_code=status.HTTP_403_FORBIDDEN)
...
class LaboratorianCompleteLabOrderItemsView(APIView):
	...
	def post(self, request, lab_order_id):
		if not is_approved_laboratorian(request.user):
			return error_response("Only approved laboratorians can complete tests.", status_code=status.HTTP_403_FORBIDDEN)
		...
		return success_response("Items processed.", data=LabOrderLaboratorianScanSerializer(data).data)
...
class LabResultCreateView(APIView):
	...
	def post(self, request, lab_order_item_id):
		if not is_approved_laboratorian(request.user):
			return error_response("Only approved laboratorians can create lab results.", status_code=status.HTTP_403_FORBIDDEN)
		...
		return success_response("Lab result created.", data=LabResultSerializer(lab_result).data, status_code=status.HTTP_201_CREATED)
	...
	def get(self, request, lab_result_id):
		...
		if request.user.id == lab_result.patient_id:
			if lab_result.status != LabResultStatus.RELEASED:
				return error_response("Not found.", status_code=status.HTTP_404_NOT_FOUND)
			return success_response("Lab result retrieved.", data=LabResultPatientSerializer(lab_result).data)
	...
	def get_queryset(self):
		if request_user := getattr(self.request, "user", None):
			if request_user.user_type == UserType.PATIENT:
				return LabResult.objects.filter(patient=request_user, status=LabResultStatus.RELEASED).select_related("lab_order_item")
		return LabResult.objects.none()

	def list(self, request, *args, **kwargs):
		if request.user.user_type != UserType.PATIENT:
			return error_response("Only patients can list lab results.", status_code=status.HTTP_403_FORBIDDEN)
		serializer = self.get_serializer(self.get_queryset(), many=True)
...
class PatientLabResultDetailView(APIView):
	...
	def get(self, request, lab_result_id):
		if request.user.user_type != UserType.PATIENT:
			return error_response("Only patients can access this endpoint.", status_code=status.HTTP_403_FORBIDDEN)
		lab_result = get_object_or_404(LabResult, id=lab_result_id, patient=request.user, status=LabResultStatus.RELEASED)
		return success_response("Lab result retrieved.", data=LabResultPatientSerializer(lab_result).data)
...
class LabResultCorrectionView(APIView):
	...
	def post(self, request, lab_result_id):
		...
		try:
			lab_result = correct_lab_result(lab_result, request.user, new_data, reason, request=request)
		except (ValueError, PermissionError) as exc:
			return error_response(str(exc), status_code=status.HTTP_400_BAD_REQUEST)
...
class DoctorLinkLabResultToMedicalRecordView(APIView):
	...
	def post(self, request, lab_result_id):
		lab_result = get_object_or_404(LabResult, id=lab_result_id)
		try:
			lab_result = link_lab_result_to_medical_record(lab_result, request.user, request=request)
		except (ValueError, PermissionError) as exc:
			return error_response(str(exc), status_code=status.HTTP_400_BAD_REQUEST)
		return success_response("Lab result linked to medical record.", data=LabResultSerializer(lab_result).data)
